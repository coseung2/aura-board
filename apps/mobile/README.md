...
- 비밀번호 및 지원 provider OAuth 로그인, 앱 딥링크 콜백 (이메일 매직링크는 폐기)
- `홈 / 알림 / 추가 / 계정` 4탭
...
학생 홈·보드 목록 API의 `productCapabilities`와 `availableLayouts`를 기준으로
화면과 네비게이션을 구성합니다. 놀이·피드·개발중 레이아웃은 관리자 학급 전용이며,
권한 정보가 없는 구버전 캐시는 다시 검증합니다. 현재 앱은 새 계약을 제공하는
서버가 먼저 배포된 뒤 적용해야 합니다. 아래 표는 구현 지원 목록이지 일반 공개 목록이 아닙니다.
