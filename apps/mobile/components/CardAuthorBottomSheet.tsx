...
export function CardAuthorBottomSheet({
  ...
  const initialStudentIds = useMemo(
    () => initialAuthors.flatMap((author) => (author.studentId ? [author.studentId] : [])),
    [initialAuthors],
  );
  ...
  useEffect(() => {
    ...
    let active = true;
    setLoading(true);
    void apiFetch<{ students: RosterStudent[] }>(`/api/classroom/${classroomId}/students`)
      .then((response) => {
  ...
  const closeOnDrag = useMemo(
    () =>
      PanResponder.create({
        onMoveShouldSetPanResponder: (_event, gesture) =>
          gesture.dy > 6 && Math.abs(gesture.dy) > Math.abs(gesture.dx),
        onPanResponderRelease: (_event, gesture) => {
          if (gesture.dy > 56 || gesture.vy > 1.1) onCloseRef.current();
        },
      }),
    [],
  );
  ...
  const selectedAuthors = useMemo(() => {
    ...
    return selectedIds.flatMap((studentId) => {
      ...
      return student ? [{ studentId: student.id, displayName: student.name }] : [];
    });
  ...
  async function save() {
    ...
    setError(null);
    try {
      const response = await apiFetch<AuthorsResponse>(`/api/cards/${cardId}/authors`, {
        method: "PUT",
    ...
    >
      <View style={styles.root}>
        <ControlPressable
          ...
          onPress={onClose}
          accessibilityLabel="작성자 선택 닫기"
        >
          <View />
        </ControlPressable>
        <View style={styles.sheet} accessibilityViewIsModal>
          <View {...closeOnDrag.panHandlers} style={styles.handleArea}>
                ...
                return (
                  ...
                  >
                    <View style={styles.studentText}>
                      ...
                      {student.number !== null ? <Text style={styles.studentNumber}>{student.number}번</Text> : null}
                    </View>
                    <Text style={[styles.check, selected && styles.checkSelected]}>{selected ? "✓" : "○"}</Text>
                  </ControlPressable>
                );
              })}
              {!students.length && !error ? <Text style={styles.empty}>등록된 학생이 없어요.</Text> : null}
            </ScrollView>
          )}

          {error ? <Text style={styles.error} accessibilityRole="alert">{error}</Text> : null}
          <View style={styles.actions}>
            <AppButton variant="secondary" onPress={onClose} style={styles.actionButton}>취소</AppButton>
            <AppButton
              onPress={save}
              loading={saving}
...
const styles = StyleSheet.create({
  ...
  handle: { width: iconSizes.xl, height: spacing.xs, borderRadius: radii.pill, backgroundColor: colors.borderHover },
  header: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: spacing.lg, paddingBottom: spacing.sm },
  title: { ...typography.title, color: colors.text },
  summary: { ...typography.label, color: colors.textMuted, marginTop: spacing.xxs },
  closeButton: { minWidth: tapMin, paddingHorizontal: spacing.sm },
  state: { minHeight: tapMin * 4, alignItems: "center", justifyContent: "center", gap: spacing.sm },
  ...
  studentRow: { minHeight: tapMin, flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: spacing.md, borderWidth: borders.hairline, borderColor: colors.border, borderRadius: radii.btn, backgroundColor: colors.surface },
  studentRowSelected: { borderColor: colors.accent, backgroundColor: colors.accentTintedBg },
  studentText: { flexDirection: "row", alignItems: "baseline", gap: spacing.sm, flexShrink: 1 },
  ...
  error: { ...typography.label, color: colors.danger, paddingHorizontal: spacing.lg, paddingBottom: spacing.sm },
