...
const checks = [
  ...
  { name: "rgba color", pattern: /rgba\(/ },
  {
    name: "legacy shadow",
    pattern:
      /\b(?:shadowColor|shadowOffset|shadowOpacity|shadowRadius|elevation):/,
  },
...
const allowed = [
  ...
  {
    file: "components/ui.styles.ts",
    patterns: [/navigation\.headerHeight/],
  },
  {
...
tokenLines.forEach((line, index) => {
  if (/\bletterSpacing:/.test(line) && !/\bletterSpacing:\s*0\b/.test(line)) {
    failures.push(
      `theme/tokens.ts:${index + 1} nonzero token letter spacing: ${line.trim()}`,
    );
  }
  if (
    /\broomCodeLetterSpacing:/.test(line) &&
    !/\broomCodeLetterSpacing:\s*0\b/.test(line)
  ) {
    failures.push(
      `theme/tokens.ts:${index + 1} nonzero room code letter spacing: ${line.trim()}`,
    );
  }
});

const sharedUiSource = [
  readFileSync(join(root, "components", "ui.tsx"), "utf8"),
  readFileSync(join(root, "components", "ui.styles.ts"), "utf8"),
].join("\n");
if (
  !/textFieldMultiline:\s*\{[\s\S]*?minHeight:\s*controls\.multilineInputMinHeight/.test(
    sharedUiSource,
  )
) {
  failures.push(
    "components/ui.tsx multiline TextField must use controls.multilineInputMinHeight",
  );
}
if (
  !/appHeaderBack:\s*\{[\s\S]*?backgroundColor:\s*colors\.transparent/.test(
    sharedUiSource,
  )
) {
  failures.push(
    "components/ui.tsx AppHeader back control must remain flat and transparent",
  );
}
  ...
  for (const filePath of walk(fullRoot)) {
    ...
    lines.forEach((line, index) => {
      ...
      if (/\bzIndex:/.test(line) && !/\bzIndex:\s*layers\./.test(line)) {
        failures.push(
          `${rel}:${index + 1} non-semantic z-index: ${line.trim()}`,
        );
      }
    });
  }
