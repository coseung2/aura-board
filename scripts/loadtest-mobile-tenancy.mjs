...
const TARGET = (env.LOADTEST_TARGET ?? "https://aura-board.com").replace(/\/$/, "");
const RUN_ID = required("LOADTEST_RUN_ID");
const TEACHER_USERNAME = required("LOADTEST_TEACHER_USERNAME");
const TEACHER_PASSWORD = required("LOADTEST_TEACHER_PASSWORD");
const PROFILE = env.LOADTEST_PROFILE === "smoke" ? "smoke" : "full";
const CLASSROOM_OFFSET = integerEnv("LOADTEST_CLASSROOM_OFFSET", 0, 0, 999);
const CLASSROOMS = integerEnv("LOADTEST_CLASSROOMS", PROFILE === "smoke" ? 2 : 50, 1, 100);
const STUDENTS_PER_CLASS = integerEnv("LOADTEST_STUDENTS_PER_CLASS", 20, 1, 100);
const STUDENT_CODE_PREFIX = env.LOADTEST_STUDENT_CODE_PREFIX ?? "X8";
const ARRIVAL_MS = integerEnv("LOADTEST_ARRIVAL_MS", PROFILE === "smoke" ? 2_000 : 20_000, 0, 120_000);
const MIXED_ACTIONS = integerEnv("LOADTEST_MIXED_ACTIONS", 4, 0, 20);
const TIMEOUT_MS = integerEnv("LOADTEST_TIMEOUT_MS", 15_000, 1_000, 120_000);
const RETRY_LIMIT = integerEnv("LOADTEST_SHADOW_RETRY_LIMIT", 35, 1, 100);
const OMOK_POLL_ATTEMPTS = integerEnv(
  "LOADTEST_OMOK_POLL_ATTEMPTS",
  PROFILE === "smoke" ? 2 : 6,
  1,
  20,
);
const OMOK_POLL_BASE_MS = integerEnv("LOADTEST_OMOK_POLL_BASE_MS", 750, 100, 30_000);
const OMOK_POLL_MAX_MS = integerEnv("LOADTEST_OMOK_POLL_MAX_MS", 6_000, 100, 60_000);
const READ_P95_BUDGET_MS = integerEnv("LOADTEST_READ_P95_MS", 1_500, 100, 60_000);
const WRITE_P95_BUDGET_MS = integerEnv("LOADTEST_WRITE_P95_MS", 2_500, 100, 60_000);
const RESULT_DIR = env.LOADTEST_RESULT_DIR ?? "tmp/loadtests";
...
function jitter(baseMs, spreadMs, seed = Math.random()) {
  return baseMs + Math.floor(seed * spreadMs);
}

function requestId(prefix, actor, attempt = 0) {
...
async function http(operation, path, options = {}) {
