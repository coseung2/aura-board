...
쉐브론, 상태 표시, 학생 이름처럼 한 줄에 붙는 글리프는 텍스트와 **같은 캡하이트 박스**를 공유한다. 페이지별 `margin-top`으로 눈대중 보정하지 않는다.

| 토큰 | 값 | 의도 |
|---|---|---|
| `--icon-inline-size` | `0.62em` | 인접 글자 캡하이트에 맞춘 글리프 한 변 |
| `--icon-inline-gap` | `0.45em` | 글리프와 텍스트 사이 간격 |
| `--text-tight-leading` | `1.2` | 한 줄 라벨/이름 행의 타이트한 행간 |

공용 클래스:

- `.ds-inline-mark`: 글리프와 텍스트를 `inline-flex` + `align-items: center`로 묶는다.
- `.ds-inline-mark-icon`: 글리프 박스를 `1em` 기준 정사각으로 고정한다.
- `[data-glyph="chevron"]`: 접힘 `-45deg`, 펼침 `45deg`. `[open]` 또는 `aria-expanded="true"`에서 회전한다.

적용 규칙:

- 테이블/리스트 요약행의 토글 아이콘과 이름, 번호는 반드시 이 패턴을 쓴다.
- 히트 영역은 부모 컨트롤(`summary`, 버튼)이 담당한다. 아이콘 자체에 별도 오프셋을 주지 않는다.
- 새 화면에서 같은 정렬 문제가 보이면 페이지 CSS를 고치지 말고 이 유틸을 재사용한다.
