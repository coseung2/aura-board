...
이 PNG 세트는 2026-09-08 디자인 산출물이다. 이후 웹·모바일 구현에 적용되었으므로
현재 동작의 기준은 구현 코드이며, 클릭 프로토타입은 여전히 포함되지 않는다.
v2 교사 플로우와 꼬들 페이지는 이 문서가 아니라
[figma-manifest.json](figma-manifest.json)의 `pages`를 참조한다.
