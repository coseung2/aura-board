...
└─ Feature Page
   ├─ Design Notes
   ├─ [WIP] → Screen Frames
   ├─ [REVIEW] → Screen Frames
   ├─ [READY] → Screen Frames
   └─ [DEPRECATED] → Screen Frames
```
...
Canonical identity is (feature, role, platform, breakpoint, UI state). Keep
one canonical READY representation per identity when approval and implementation
alignment are evidenced; do not force incomplete candidates into READY.
An approved historical screen retains its approval provenance even when archived.
Record missing states as gaps rather than generating replacements during cleanup.
...
6. archive superseded alternatives while preserving approval history and node IDs.
   Deletion is a separate explicitly authorized action.
...
**READ → CODE COMPARE → MIGRATION PLAN → FIGMA WRITE → SCREENSHOT VERIFY**
...
## 16. Handoff verification

Use [the project verification checklist](../verification-checklist.md#figma-design-operations)
as the single verification checklist. Record results and remaining gaps in the
feature handoff or migration record; do not maintain competing checklists here.
...
## 19. Migration evidence and decision boundaries

Follow the safe migration rules in the global Design Ops document. Record each
batch in a dated file under `docs/design/`: inspected file/page IDs, source code
revision and working-tree status, canonical identity and selection evidence,
approval provenance, old/new parent and node IDs, component/variable consumers,
rollback instructions, screenshots inspected, and unresolved discrepancies.

The source hierarchy describes the implementation baseline, not authority to
overwrite approved product intent. Explicit user decisions govern scope.
Distinguish committed implementation, uncommitted work, and verified deployment.
If no existing frame matches a current state, retain the nearest approved
reference with a documented mismatch; do not redesign it during a cleanup task.
Uncertain candidates remain REVIEW until evidence resolves their status.

Use in-file sections for incremental migration. A shared-library file split,
library publication, deletion of approved assets, or broad rebinding is not
implied by routine organization. Preserve existing component IDs and instance
overrides whenever feasible. Compare semantic purpose, variant properties,
layout behavior and token modes before merging; keep historical masters until
all known consumers migrate and external consumption is understood.

Registration: repository `AGENTS.md` must load this constitution and
`docs/ai/global-figma-design-ops.md` for Figma tasks. The global installation at
`C:/Users/user/.codex/FIGMA_DESIGN_OPS.md` is an identical copy of the reusable
document, loaded by the global `AGENTS.md`. Update both copies together when
changing the global policy; this constitution owns only project-specific rules.
