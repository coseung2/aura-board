...
| Supabase Free (DR) | 논리 replica, DR용 Postgres·PostgREST·RLS·Realtime warm standby | Oracle primary write path, 자동 promotion, object payload | Seoul `aura-board-dr`에서 2026-08-30 current Oracle `postgres` 기준 176-table full copy와 row-count exact parity, heartbeat fresh를 재검증. Storage는 Free 한계를 넘어 명시적 media degraded mode 사용 |
| GitHub Actions | 운영 cron endpoint 수동 실행과 장애 시 대체 호출 | 앱 호스팅, 기본 스케줄러 | 8개 endpoint용 수동 workflow를 비상 운영 경로로 유지 |
| Vercel | 전환 기간의 마지막 검증 가능한 배포본, Supabase Free warm standby용 DR runtime | Oracle primary의 신규 운영 트래픽, 기본 cron scheduler | `mallagaenge-1872's projects/aura-board-dr` production deployment `dpl_CiNbYJYtybN5GuCTef7FXYwVfAfv`가 `READY`이며 alias `/api/health`는 `200`, DB reachable, replication fresh를 반환한다. 평상시 운영 DNS와 scheduler에서는 제외 |
...
- [x] 2026-08-20 backup hardening 재검증: `restore-rehearsal.sh`를 Supabase PG17 ARM64 digest pin, staged-byte checksum binding, 최소 capability, no-network/read-only/bounded tmpfs·timeout, label-verified cleanup으로 자동화했다. A1의 실제 extension-bearing full dump를 `--exit-on-error`로 격리 복원해 성공하고 잔존 restore container 0개를 확인했다. `offsite-backup.mjs`의 AES-256-GCM+scrypt round trip과 wrong-passphrase/tamper/malformed-header/KDF/symlink/destination-race/no-overwrite behavioral test를 통과했다. 실제 Oracle 외부 저장소로의 encrypted artifact 전송은 아직 수행하지 않았다.
...
운영 기준과 상세 설계는 [`docs/supabase-selfhost-dr.md`](supabase-selfhost-dr.md)를 참조하고, 아래 항목은 이 handoff와 [`docs/verification-checklist.md`](verification-checklist.md)의 공통 acceptance evidence로 사용한다. 각 증거에는 대상(project/endpoint), UTC 시각, commit/deployment SHA 또는 SQL/log artifact를 기록하고 secret 값은 기록하지 않는다. DB/API/Realtime와 media degraded-mode gate, Vercel deployment 및 DR `/api/health` 검증은 완료했으며 production traffic switch·failover/failback은 미완료다.

DR 프로젝트 이름은 `aura-board-dr`, 리전은 Seoul(`ap-northeast-2`)이다. `coseung-sk` 전용 CLI profile을 인증해 ACTIVE_HEALTHY 상태로 연결했고 secret은 Infisical의 DR 전용 scope에 저장한 뒤 로컬 평문 사본을 제거했다. Oracle publisher와 Supabase subscriber의 실제 initial copy 및 지속 추종, schema/RLS/PostgREST/Realtime 검증을 완료했다. Vercel에는 별도 `aura-board-dr` project를 `mallagaenge-1872's projects` 팀에 생성하고 framework preset을 Next.js로 적용했으며 production-only runtime env를 주입했다. exact-SHA production deployment와 외부 health 검증까지 완료했다.

- [x] Schema/data parity: 2026-08-30 missing Prisma migrations 4개를 적용하고 source publication 176 tables 모두 destination 존재를 확인. full initial copy 뒤 146/146 migration history와 176-table row counts exact, mismatch 0.
- [x] Logical replication lag/heartbeat: current `postgres`의 `aura_board_oracle_dr_slot_v2` active, subscription 176/176 relation `ready`, apply error 0, private heartbeat fresh 확인. 과거 sync error 1은 missing table grant 복구 전 기록이다.
...
- [x] Vercel deployment health: credential rotation 후 production deployment `dpl_CiNbYJYtybN5GuCTef7FXYwVfAfv`가 `READY`이며 alias `/api/health`는 `200`, database reachable, replication fresh를 반환했다. 이전 degraded notice와 auth-first upload `401` acceptance는 유지된다.
...
- Storage S3 Customer Secret과 S3 backend 설정 8개는 A1 root-owned mode `0600` `.env`를 source로 Infisical `prod` `/oracle/aura-board/supabase-storage`에 값 노출 없이 동기화했고, 원본과 exact 비교 후 임시 transfer 파일을 제거했다. 실제 credential rotation/recovery rehearsal은 별도 운영 작업으로 유지한다.
...
- Vercel DR project는 Next.js preset, production-only env 42개, DR DB/Supabase 값, media degraded/replication health flag, exact-SHA production deployment와 외부 health까지 검증됐다. Cloudflare origin switch와 subscriber promotion은 별도 승인 전까지 수행하지 않는다.
...
| 일자 | 상태 | 기록 | 다음 단계 |
| --- | --- | --- | --- |
| 2026-08-21 | Production Oracle self-hosted cutover 완료 | Managed production password rotation과 read-only/pg_cron fence 후 export, candidate restore/verify, Oracle DB promotion, app/backup runtime env 전환, exact release switch 및 seal을 완료했다. Library migration과 Prisma history 147개가 promoted DB에 유지됐고 로그인된 `/teacher/library`가 정상 렌더링됐다. Infisical selfhost scope에는 `POSTGRES_USER`와 loopback direct port metadata를 보완했다. app/play-engine/backup timer 및 self-host API가 healthy이며 외부 health는 `200`/DB reachable다. `Deploy Oracle Production` workflow를 재활성화하고 run `32483473309`로 main SHA `1049cf64...` ARM64 build/deploy를 완료했다. | 별도 DR project의 잘못 reset된 DB password consumer를 해당 project owner profile로 갱신. 이번 live 실행에서 발견해 local 수정한 cutover parser/restore/runtime 계약 결함을 review 후 commit/push. managed source의 보존·폐기 정책을 정한 뒤 별도 승인된 절차로 정리 |
| 2026-08-21 | Production cutover live preflight 완료, 계정 권한에서 안전 중단 | cutover 도구 SHA `19cd0cd6`을 A1에 배포하고 Docker 실제 PostgreSQL 포함 102 tests를 통과했다. loopback DB proxy와 `supabase_admin` restore credential로 Auth/application catalog, Prisma 147개, data, target DB owner/ACL live preflight를 완료했고 target owner/ACL 37개 객체를 source와 exact 정렬했다. maintenance를 열었으나 현재 Whale/CLI Supabase 계정이 contract production project를 소유하지 않아 source password reset 전에 중단했다. 모든 service/cron/container를 복원했고 외부 health `200`, production old credential 유효, temporary credential 거부를 확인했다. 별도 `aura-board-dr` project password를 잘못 reset했으므로 그 project credential consumer도 정리해야 한다. Production fence/export/candidate/promotion/runtime switch/seal은 미실행이고 deploy workflow는 계속 수동 비활성화 상태다. | contract production project를 소유한 Supabase account로 Whale에 로그인. production password reset 후 read-only/cron → `adopt-fence`부터 재개. Infisical `MANAGED_DB_PROJECT_REF`를 contract suffix와 맞추고 잘못 reset한 별도 DR project password consumer를 동기화. 완료 후 active SHA/health 확인 및 deploy workflow enable |
| 2026-08-21 | Production cutover preflight 직전에서 안전 중단 | Library migration과 Prisma history 147개를 managed/Oracle에 맞추고, hardened cutover 도구와 live Envoy compose container 계약을 SHA `4e8389ec`으로 push했다. self-host 공개 env로 fresh app/play-engine release와 mode-0600 build manifest를 A1에 staging하고, Infisical `prod /oracle/aura-board/cutover/selfhost` 75개 값 및 root-only contract/state directory를 준비했다. managed Bastion SSH와 private subnet TCP/22 경로를 복구했으며 임시 public target SSH rule과 작업용 Bastion client `/32`는 제거했다. source old credential probe는 성공했고 temporary credential은 rotation 전이라 예상대로 거부됐다. Oracle Docker internal DB로의 target/admin transport가 실패해 preflight 이전에 중단했으며 write fence, password rotation, env switch, promotion, seal은 미실행이다. `Deploy Oracle Production` workflow는 수동 비활성화했다. | `docs/production-cutover-runbook.md`의 재개 체크포인트를 따라 승인된 현재 client `/32`로 Bastion을 연결하고 loopback-only `127.0.0.1:15434 -> supabase-db:5432` admin proxy를 준비한 뒤 contract의 target/admin endpoint만 갱신해 preflight부터 재실행. preflight exact match 전에는 maintenance/fence를 시작하지 않으며, cutover 완료·active SHA·health 확인 후 deploy workflow를 다시 enable |
| 2026-08-21 | Supabase Free + Vercel warm standby 배포 검증 완료 | Oracle workflow run `32389558184`에서 ARM64 root replication installer 22개 행동 테스트, Next.js/Rust build, exact SHA `f35286e1` app/play-engine 배포와 내부 health가 통과했다. 동일 SHA metadata로 Vercel deployment `dpl_5Hu4bGn2aBDEut8YJop2qUt2R3zo`를 production `READY`/`icn1`에 배포했고 alias `/api/health`의 DB reachable·replication fresh, degraded notice, auth-first upload `401`, error log 0건을 확인했다. | stable replication/Supabase DNS credential 확보 후 endpoint 교체 검증. 별도 승인된 write fence·Cloudflare origin switch·failover/failback rehearsal 전까지 warm standby로 유지 |
| 2026-08-20 | self-hosted Supabase Storage·Realtime·Bastion staging 경로 검증 완료 | private/versioned OCI bucket `aura-board-storage`, bucket-scoped IAM group/user/policy와 S3 Customer Secret을 구성하고 Storage container를 OCI S3 HTTPS/path-style backend로 recreate해 healthy 확인. managed payload 1,226건/1,040,594,444 bytes 이관, OCI count/bytes exact match, direct S3 SHA-256 8/8 및 self-hosted Storage API download SHA-256 8/8 통과. URL inventory는 11 column/1,173 row이며 `supabase.aura-board.com` + gradual backfill로 결정. Realtime Broadcast 실제 publish/subscribe 성공. Bastion `404`는 `read private-ips` 누락으로 해결하고 최소 권한 session create/reuse, client `/32` allowlist, local port-forwarding A1 SSH를 검증한 뒤 public `0.0.0.0/0:22`를 private subnet TCP/22로 교체. 외부 TCP/22 closed, Bastion SSH와 HTTPS health `200`. production source of truth와 endpoint는 managed Supabase 유지. | Storage S3 credential을 Infisical의 승인된 Oracle scope에 동기화하고 rotation/recovery를 검증한 뒤 `supabase.aura-board.com` public endpoint/nginx/DNS 및 download/CORS를 검증. Supabase Free `aura-board-dr` 생성·복제·Vercel 배포는 다음 acceptance 기록에서 완료 |
