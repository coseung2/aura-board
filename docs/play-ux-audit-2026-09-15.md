...
- 모바일 typecheck는 변경하지 않은 공통 shadow 스타일 타입과 expo-router/react-navigation 해석 오류가 남았다. 최종 단계에서 의존성/공통 타입 기준선을 별도 검토한다. 실제 두 클라이언트 지연·기기 화면 및 운영 DB 전파는 아직 실측하지 않았다.
