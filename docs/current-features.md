...
- `/parent/(app)/feed` — 연결된 모든 자녀 게시물의 최신순 통합 피드
- `/parent/(app)/home` — 자녀 선택 + 미디어/텍스트 게시물 그리드

### 주요 동작
- OAuth 첫 로그인: provider 인증 → 신규 Parent + ParentOAuthAccount → onboard/match/code 진입
- OAuth 재로그인: provider 인증 → 기존 Parent 즉시 사용 → 통합 피드
- email 매칭으로 기존 매직링크 학부모 자동 link (verified email 만)
- 다자녀: 홈의 자녀 프로필 탭으로 자녀별 게시물 전환
- 자녀 추가: 홈의 자녀 추가 유틸리티 → /parent/onboard/match/code
- 홈 그리드의 게시물 선택 → `/parent/feed?post=ID`로 이동해 해당 게시물에 초점

### 권한
- ParentOAuthAccount: (provider, providerAccountId) unique 로 OAuth 계정 = Parent 1:1
- 자녀 외 학생 게시물: 0건 누출 (활성 ParentChildLink 범위 가드)
