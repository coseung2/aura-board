...
- Canva Developer Terms의 미디어 처리 조항은 원본 미디어와 그 사본·파생물을 접근 후 5분 이내 삭제하고, 한 번에 하나의 원본 미디어만 처리하도록 요구한다. 현재 PDF의 `A4 자동 배치`는 여러 디자인을 한 페이지에 결합하므로 Canva의 서면 확인 또는 법률 검토 전에는 적합하다고 단정하지 않는다.
- 이 항목은 문구만 추가해서 해결하지 말고 실제 기능 동작을 함께 확정한다.
