...
3. 하나 이상의 디자인을 선택해 선택한 순서대로 하나의 PDF로 병합되고, 각 Canva
   원본 페이지의 크기와 내부 배치가 유지되는지 확인한다.
...
- 라이브러리는 Canva PDF 원본을 영구 저장하지 않고 디자인 ID와 표시 메타데이터만 저장한다.
- 최종 다운로드 시에만 Canva export를 수행하고, 처리 중 생성한 바이트는 응답 후 보관하지 않는다.
- 다운로드 API는 인증된 교사가 소유한 라이브러리 항목만 허용하고 최대 20개를 처리한다.
