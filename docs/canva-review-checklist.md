...
출력의 `teacherUpdates`와 `studentUpdates`만 운영 기록에 남기고 토큰 값은
기록하지 않는다.
...
3. 선택한 디자인을 PDF로 내보낸다.
4. 보드 섹션 이름으로 Canva 폴더를 만들고 디자인을 이동한다.
5. 심사용 학생 계정으로 로그인하고 `/my/wallet`에서 Canva를 연결한다.
6. Aura 카드 생성을 실행해 QR 이미지 에셋과 편집 가능한 Canva 디자인이 생성되는지 확인한다.
7. 교사와 학생 각각 연결 해제를 실행한다.
8. 새로고침 후 연결 해제 상태가 유지되고, 다음 Canva 작업에서 재인증을 요구하는지 확인한다.
...
사용하지 않는 comment, permission, brand template scope는 요청하지 않는다.

## OWASP Top 10 검토

이 검토는 코드 기반 보안 검토이며 외부 침투 테스트를 의미하지 않는다.

- A01 접근 통제: 연결 상태와 해제 API는 현재 로그인한 교사 또는 학생의 연결 행만 다룬다.
