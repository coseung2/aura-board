...
- Render the teacher dashboard 놀이 tab and confirm it shows 잼라이브 plus exactly Shadow Alliance, Kordle, Speed Game, Omok, and Song Guess. Confirm the teacher must select an owned classroom before opening an official game, the resulting room is the same classroom-owned room students enter, the teacher header no longer has a separate live-quiz icon, and no teacher-authored legacy official-game board appears in the dashboard or top navigation.
