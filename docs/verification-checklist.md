...
- For all five PLAY games, distinguish ephemeral `현재 접속` from durable `참가` and game-specific waiting. Open and close the same game on web and Expo and confirm classroom-scoped Presence appears and disappears without rewriting score/participant history. For Omok, start random matchmaking or host a public room, then leave/background the lobby: the waiting lease must cancel immediately on the clean path and expire without revival after its TTL on the crash path; reopening the lobby must not heartbeat an already expired ticket. The hub must label Omok queue counts as `대기`, durable game membership as `참가`, and current Presence separately as `접속`. For Kordle, keep board-scoped lobby Presence separate from durable LIVE attempts. For Song Guess, Speed Game, and Shadow Alliance, simple screen exit must not be treated as proof that durable participation should be deleted; their leave/forfeit/terminal rules remain authoritative.
...
#### PLAY UX lifecycle and disclosure checks (2026-09-15)

- Hub/room state must follow committed lifecycle state, not merely `current`, seeded roster size, or the latest-created session. Check simultaneous free rooms, reveal vs lobby, completed and host-ended rooms, teacher exclusion from student counts, and delayed responses. A participant count is not an online-presence measurement.
- Verify first-subscribe catch-up, healthy Broadcast updates without periodic list polling, recovery after a failed HTTP reconciliation on a healthy channel, and server-deadline-driven catch-up when no gameplay screen remains open.
- Finish an Omok room via HTTP and the authoritative socket. Active room/tickets must retire without deleting receipts/results. Repeat cleanup and replay an old session event after a rematch; predicates must not affect the new session. Outbox cleanup failure must remain retryable.
- During guessing, public score/rank and round gain must stay at the last revealed result. Correct counts and multiple-choice command results must not reveal correctness. Refresh, stored receipt replay, teacher/projector and student views must respect the same boundary. Reveal publishes the gains once; private text-answer locking remains separate.
- Apply `20260915103000_song_guess_round_catalog_identity` before deploying source-preserving teacher setup. Catalog materialization and manual save of an existing clip retain trusted provenance. Participant payloads must not contain catalog identities or answer-source metadata.
- Distractors must share the round's category and remain distinct after title/artist/alias normalization. Test mixed-category selections and insufficient same-category pools; never silently fill them from unrelated categories. Category-less hand-authored packs use only their own authored alternatives. The stable request/round seed must keep order and IDs deterministic.
...
- Automatic choices may read full-catalog metadata, but select distractors only from the matching category and selected pack's eligible alternatives. A single selected song must produce one question with its answer and three distinct distractors when that category has enough alternatives; otherwise report the shortage. Exclude normalized duplicate titles and alias overlap, including aliases shared between two distractors. Text mode must not query the choices catalog. Catalog metadata does not require downloading distractor audio.
