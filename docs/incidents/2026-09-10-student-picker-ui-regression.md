...
## 2026-09-16 재발: 상단 장식 때문에 뽑기 화면 잘림

- 증상: 히어로, 단계 표시, 학급 설명 카드가 세로 공간을 차지해 인원 선택, 뽑기 대상, 뽑기 화면, 뽑기 버튼이 잘렸다.
- 영향: 교사가 뽑기 조작을 한 화면에서 볼 수 없었다.
- 확인 원인: 장식용 상단과 단계 UI가 본문보다 먼저 고정 높이를 썼다.
- 대응: 상단 히어로, 단계 표시, 효과음 토글, 안내 카드, 하단 결과 요약을 제거했다. 이동/닫기와 학급 드롭다운만 남기고 인원 선택, 뽑기 대상, 뽑기판, 뽑기 버튼을 본문으로 올렸다. 기본 창 높이는 560으로 줄였다.
- 복구 검증: StudentRandomPickerPanel.vitest.tsx 8개 통과. git diff --check 통과.
## 2026-09-16 재발: 이동 창 구조와 크기 조절 누락

- 증상: 학생뽑기 창이 화면 중앙이 아닌 FAB 주변에서 열리고, 별도 이동 막대가 세로 공간을 차지했다. 창 크기를 조절할 수 없었으며 낮은 화면에서는 내부 콘텐츠가 작은 패널 안에서 잘린 것처럼 보였다.
- 영향: 교사가 명단, 추첨 버튼, 결과를 한 화면에서 확인하기 어려웠고 요청된 중앙 배치·이동·크기 조절 창 동작이 완성되지 않았다.
- 확인 원인: `useToolkitMovement`가 기존 CSS의 우측 하단 위치를 최초 위치로 사용하고 뷰포트 최대 크기만 제한했다. 이동 전용 행은 추가됐지만 창 크기 상태와 resize 입력은 구현되지 않았다.
- 대응: 최초 크기를 뷰포트에 맞춘 640×760으로 계산해 중앙에 배치하고, 이동 손잡이를 기존 헤더 안으로 옮겼다. 우하단 크기 조절 손잡이를 추가했으며 헤더·진행 단계는 고정하고 본문만 스크롤되도록 창 구조를 수정했다.
- 복구 검증: `StudentRandomPickerPanel.vitest.tsx` 8개와 전체 `npm run typecheck`가 통과했다. 실제 컴포넌트와 CSS를 헤드리스 Chrome에서 렌더링해 768×1024에서 640×760 중앙 배치, 932×430에서 뷰포트 내 391×406 배치와 본문 스크롤을 확인했다. 같은 화면을 회전해도 선택 상태가 유지됐고, 하단 추첨 버튼과 결과는 본문 스크롤 후 모두 창과 뷰포트 안에 들어왔으며 가로 오버플로는 없었다. 포인터 크기 조절과 키보드 이동 경계는 컴포넌트 테스트로 확인했다.
