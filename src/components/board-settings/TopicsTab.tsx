  ...
  async function save() {
    ...
    try {
      ...
      const latestById = new Map(
        sectionsRef.current.map((section) => [section.id, section] as const),
      );
      setDraft((current) =>
        mergeDraftMetadata(
          mergeSectionPositions(current, confirmed),
          latestById,
        ),
      );
...
function mergeDraftMetadata(
  ...
  latestById: ReadonlyMap<string, BoardSection>,
): EditableSection[] {
  return draft.map((row) => {
    ...
    return {
      ...row,
      ...latest,
      accessToken: latest.accessToken ?? null,
      baseOrder: latest.order ?? row.baseOrder,
      order: row.order,
      pinned: row.pinned,
    };
  });
