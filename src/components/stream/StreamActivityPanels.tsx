...
export function MapActivityPanel({ sectionId, canEdit }: { sectionId: string; canEdit: boolean }) {
...
export function buildWindowOpeningCells(
...
export function groupWindowOpeningCards(cards: CardData[], cells: WindowOpeningCell[]) {
...
export function buildWordCloud(cards: CardData[]) {
...
export function normalizeWordCloudEntry(value: string | null | undefined): string {
  return limitWordCloudInput(value ?? "").trim();
}

export function limitWordCloudInput(value: string): string {
...
export function buildTimeline(cards: CardData[]) {
  return cards
    .map((card) => {
      const text = `${card.title} ${card.content}`;
      const dateText =
