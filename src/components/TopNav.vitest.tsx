...
describe("TopNav classroom mega menu", () => {
  it("renders the four visible groups with classroom links and canonical hrefs", async () => {
    ...
    expect(within(panel).getByText("자리·모둠")).toBeTruthy();
  ...
  it("shows seating without an administrator tag", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => new Response(JSON.stringify(classroomNavData))),
    );
    ...
    openClassroomMenu();
    const panel = await screen.findByRole("region", { name: "학급 메뉴" });
    const link = within(panel).getByRole("link", { name: "자리·모둠" });
    expect(link).toHaveAttribute("href", "/classroom/class-1/groups");
    expect(within(link).queryByText("관리자")).toBeNull();
  });
