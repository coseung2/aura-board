...
export function rankSongGuessParticipants(participants: Participant[]) {
  const sorted = participants.map((participant, index) => ({ ...participant, originalIndex: index }))
...
export const SongGuessParticipantPet = memo(function SongGuessParticipantPet({ participant }: { participant: Participant }) {
  ...
  const items = pet ? visibleEquippedSlimeItemKeys(pet.equippedItemKeys, pet.hiddenItemKeys)
    .map((key) => getSlimeShopItem(key)).filter((item): item is SlimeShopItem => Boolean(item)) : [];
  return <div className={styles.pet} role="img" aria-label={`${participant.displayName} ${slime ? "대표펫" : "대표펫 미지정"}`}>
    <div aria-hidden="true">
      {slime ? <SlimeCharacterSprite slime={slime} items={items} growthStage={pet?.growthStage} scale={1} hostBackground={false} /> : <UserRound size={28} />}
    </div>
  </div>;
});

export const SongGuessScoreboard = memo(function SongGuessScoreboard({ participants, podium = false, waiting = false, roundResults = false }: {
  participants: Participant[];
  podium?: boolean;
  waiting?: boolean;
  roundResults?: boolean;
}) {
  const ranked = useMemo(() => rankSongGuessParticipants(participants.filter((participant) => participant.joined !== false)), [participants]);
  const leaders = ranked.slice(0, 3);
  const visible = roundResults ? ranked.slice(0, 5) : ranked;
  return <section className={styles.board} data-round-results={roundResults} aria-label={podium ? "최종 순위" : waiting ? "참가 학생" : "점수판"}>
    <h2 className={styles.heading}>{podium ? <><Trophy size={24} aria-hidden="true" />최종 순위</> : waiting ? "참가 학생" : roundResults ? "라운드 순위" : "점수판"}</h2>
    {podium && leaders.length > 0 && <ol className={styles.podium} aria-label="상위 3명">
      {leaders.map((participant, index) => <li key={participant.participantId ?? participant.originalIndex} className={styles.podiumPlace} data-place={index + 1}>
    ...
    <ol className={styles.list} aria-label={podium ? "전체 순위" : waiting ? "참가 학생 목록" : "현재 순위"}>
      {(waiting ? participants.filter((participant) => participant.joined !== false).map((participant, index) => ({ ...participant, originalIndex: index, rank: 0 })) : visible).map((participant) => (
        <li key={participant.participantId ?? participant.originalIndex} className={styles.row} data-leading={!waiting && participant.rank === 1}>
          ...
          {roundResults && <span className={styles.movement} data-direction={participant.previousRank != null && participant.previousRank < participant.rank ? "down" : "up"}>
            {participant.previousRank == null || participant.previousRank === participant.rank ? <span aria-label="순위 유지">—</span> : <>
    ...
    {roundResults && ranked.length > 5 && <details className={styles.allRanks}>
      <summary>전체 순위</summary>
      <ol className={styles.list} start={6} aria-label="6위 이하 순위">
        {ranked.slice(5).map((participant) => <li className={styles.row} key={participant.participantId ?? participant.originalIndex}>
          <span className={styles.rank}>{participant.rank}위</span><SongGuessParticipantPet participant={participant} />
          <span className={styles.name}>{participant.displayName}</span><span className={styles.roundGain}>+{participant.roundScore ?? 0}</span><strong className={styles.score}>{participant.score}점</strong>
        </li>)}
      </ol>
    </details>}
