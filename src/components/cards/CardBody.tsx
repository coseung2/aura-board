...
export const CardBody = memo(function CardBody({
  ...
  const hasCanvaAttachment = Boolean(
    (card.linkUrl && extractCanvaDesignId(card.linkUrl)) ||
      card.attachments?.some((attachment) => Boolean(extractCanvaDesignId(attachment.url))),
  );
  ...
  return (
    <>
      ...
      />
      {(card.title.trim() || (isThumbnailCard && hasCanvaAttachment)) && (
        <div className="padlet-card-title-row">
          {card.title.trim() && (
            <Title className="padlet-card-title">{card.title}</Title>
          )}
          {isThumbnailCard && hasCanvaAttachment && (
            <CanvaAttribution />
          )}
        </div>
      )}
