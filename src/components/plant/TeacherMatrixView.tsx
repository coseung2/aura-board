...
export function TeacherMatrixView({ classroomId, boardId }: Props) {
  ...
  const [activeObservationId, setActiveObservationId] = useState<string | null>(
    null,
  );
    ...
    fetch(`/api/classrooms/${classroomId}/matrix`, {
      ...
      .then((j) => {
        ...
        setSelectedStudentId(
          (current) => current ?? next.students[0]?.id ?? null,
        );
      })
  ...
  useEffect(() => {
    ...
    fetch(`/api/student-plants/${obsDetail.plantId}/observations`)
      ...
      .then(({ observations }) => {
        if (cancelled) return;
        const sameStage = observations.filter(
          (obs) => obs.stageId === obsDetail.stageId,
        );
    ...
    return (
      <div className="plant-matrix-forbidden">
        ...
        <p style={{ color: "var(--color-text-muted)" }}>
          휴대폰에서는 셀이 너무 작아 보여요. 태블릿, 노트북 또는 데스크탑에서
          다시 열어주세요.
        </p>
      </div>
    );
  ...
  const selectedStage = selectedStudent?.plant
    ? (stages.find((st) => st.id === selectedStudent.plant?.currentStageId) ??
      null)
    : null;
  const selectedObservationTotal =
    selectedStudent?.cells.reduce(
      (acc, cell) => acc + cell.observationCount,
      0,
    ) ?? 0;
  const selectedPhotoStages =
    selectedStudent?.cells.filter((cell) => cell.thumbnail).length ?? 0;
  const classPhotoCells = students.reduce(
    (acc, student) =>
      acc + student.cells.filter((cell) => cell.thumbnail).length,
    0,
  );
  ...
  const detailMemo = activeObservation
    ? activeObservation.memo
    : (obsDetail?.memo ?? null);
  ...
  const detailObservedAt =
    activeObservation?.observedAt ?? obsDetail?.observedAt ?? "";

  function openObservationDetail(
    student: StudentRow,
    stage: Stage,
    cell: Cell,
  ) {
    setSelectedStudentId(student.id);
    if (!student.plant || !cell.latestObs || cell.observationCount === 0)
      return;
    setObsDetail({
    ...
    <div className="plant-matrix-shell">
      <header className="plant-matrix-toolbar">
        ...
        <div className="plant-matrix-toolbar-stats" aria-label="매트릭스 요약">
          ...
          <span>
            <strong>{stages.length}</strong>단계
          </span>
          <span>
            <strong>{classPhotoCells}</strong>개 사진 셀
          </span>
        </div>
      </header>
      ...
      <div className="plant-matrix-layout">
        <div className="plant-matrix-wrap">
          <table
            ...
            style={{
              tableLayout: "fixed",
              minWidth: students.length * STUDENT_COL_WIDTH + STAGE_COL_WIDTH,
            }}
          >
            <colgroup>
              ...
              <tr>
                ...
                {students.map((s) => (
                  <th
                    ...
                    data-selected={
                      selectedStudent?.id === s.id ? "true" : "false"
                    }
                  >
                    <button
                      type="button"
                      className="plant-matrix-student-head"
                  ...
                  {students.map((s) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isCurrent && (
                          <span
                            className="plant-matrix-current-dot"
                            aria-label="현재 단계"
                          />
                        )}
                        {cell && cell.observationCount > 0 && (
                          <span className="plant-matrix-count-badge">
        ...
        <aside className="plant-matrix-drilldown" aria-label="선택 학생 상세">
          {selectedStudent ? (
            <>
              ...
              <h3>
                {selectedStudent.number ?? "-"}번 {selectedStudent.name}
              </h3>
              ...
              <div className="plant-matrix-drilldown-stats">
                <span>
                  <strong>{selectedObservationTotal}</strong>기록
                </span>
                <span>
                  <strong>{selectedPhotoStages}</strong>사진 단계
                </span>
                <span>
                  <strong>
                    {selectedStage ? `${selectedStage.order}단계` : "대기"}
                  </strong>
                </span>
              </div>
              <div
                className="plant-matrix-mini-strip"
                aria-label="학생 단계별 기록 요약"
              >
                {stages.map((st, index) => {
                  ...
                  return (
                    <span
                      ...
                      data-filled={
                        cell && cell.observationCount > 0 ? "true" : "false"
                      }
                      data-photo={cell?.thumbnail ? "true" : "false"}
                      data-current={
                        selectedStudent.plant?.currentStageId === st.id
                          ? "true"
                          : "false"
                      }
                    >
              ...
              <p className="plant-matrix-drilldown-note">
                초록 셀은 사진 기록, 노란 셀은 글 기록만 있는 단계예요. 썸네일은
                필요한 셀만 로드해 Vercel 이미지/함수 비용을 줄입니다.
              </p>
            </>
          ) : (
            <p className="plant-teacher-empty-copy">
              학생을 선택하면 상세가 보여요.
            </p>
          )}
        </aside>
            ...
            <div className="plant-obs-detail-info">
              <div className="plant-obs-detail-head">
                <span className="plant-obs-detail-stage">
                  {obsDetail.stageOrder}단계 · {obsDetail.stageName}
                </span>
                <span className="plant-obs-detail-student">
                  {obsDetail.studentNumber ?? "-"}번 {obsDetail.studentName}
                </span>
              </div>
              {detailMemo && (
              ...
              <div
                className="plant-obs-detail-history"
                aria-label="같은 단계 관찰 기록"
              >
                <strong>이 단계 관찰 기록</strong>
                {observationsLoading && <span>불러오는 중...</span>}
                {observationsError && (
                  <span className="plant-error">{observationsError}</span>
                )}
                {!observationsLoading &&
                  !observationsError &&
                      ...
                      {stageObservations.map((obs) => {
                        ...
                        return (
                          <button
                            ...
                            data-active={
                              obs.id === (activeObservation?.id ?? obsDetail.id)
                                ? "true"
                                : "false"
                            }
                          ...
                          >
                            {thumb ? (
                              ...
                              <span className="plant-obs-detail-history-empty">
                                메모
                              </span>
                            )}
                            <span>
