...
import { isSlimeSceneBackground, SLIME_SHOP_CATALOG } from "@/lib/pets/catalog";
...
export function SlimePetPage({ initialSection = "mine" }: Props) {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      const drawer =
        shopCloseRef.current?.closest<HTMLElement>("[role='dialog']");
      if (!drawer) return;
  ...
  return (
    <main className={styles.page} data-testid="slime-pet-page">
      ...
      {initialSection === "mine" ? (
        <>
          <SlimeCollectionSection
            ...
            onEquipTitle={(color, titleKey) =>
              void equipPetTitle(color, titleKey)
            }
        ...
        <SlimePetShopDrawer
          presentation="inline"
          supportsBackground={shopCatalog.some((item) =>
            isSlimeSceneBackground(item),
          )}
          catalog={catalog}
