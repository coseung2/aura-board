...
describe("useSectionLibraryImport", () => {
  ...
  it("shows progress immediately and replaces it with the import result", async () => {
    ...
    expect(screen.getByRole("status")).toHaveStyle({ borderLeftStyle: "none" });

    await act(async () => {
      finishRequest(
        new Response(JSON.stringify({ created: 2, reused: 1, failed: 0 }), {
          status: 200,
    ...
    expect(screen.queryByText(/기존 자료/)).toBeNull();
