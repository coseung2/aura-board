...
export async function POST(request: Request) {
  ...
  if (!parsed.success || new Set(parsed.data.itemIds).size !== parsed.data.itemIds.length) {
  ...
  try {
    const sources = await inspectTeacherLibraryPrintSources({
      userId: user.id,
      items: parsed.data.itemIds.map((id) => byId.get(id)!),
      baseUrl: request.url,
    });
    return jsonPrivateNoStore({ sources });
  } catch (error) {
    if (error instanceof TeacherLibraryPdfError) {
      return jsonPrivateNoStore({ error: error.code }, { status: error.status });
    }
    console.error("[POST /api/teacher/library/print-sources]", error);
    return jsonPrivateNoStore({ error: "print_sources_failed" }, { status: 500 });
  }
}
