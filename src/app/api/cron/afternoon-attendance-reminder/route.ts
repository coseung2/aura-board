  ...
  while (true) {
    const students = await db.student.findMany({
      where: {
        ...
        // Eligibility is deliberately attendance-only. Assignment and reward
        // details enrich the digest but must never filter a student out.
        attendances: { none: { day: attendanceDate } },
        pushDispatches: {
          none: { eventKey: { startsWith: "afternoon-tasks:", endsWith: `:${day}` } },
        },
      },
      orderBy: { id: "asc" },
      select: {
    ...
    const pushes = students.map((student) => afternoonTaskReminderPush({
