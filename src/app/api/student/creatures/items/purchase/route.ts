...
import {
  ...
  itemPurchaseBodySchema,
  purchaseCreatureItem,
...
export async function POST(req: Request) {
  ...
  const parsed = itemPurchaseBodySchema.safeParse(body);
  if (!parsed.success) return jsonPrivateNoStore({ error: "invalid_body" }, { status: 400 });

  try {
    const result = await purchaseCreatureItem(
    ...
    console.error("[student/creatures/items/purchase] POST failed", error);
