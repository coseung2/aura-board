...
import { enhancePet, petErrorResponse } from "@/lib/pets/service";
...
export async function POST(_request: Request, context: { params: Promise<{ petId: string }> }) {
  ...
  try {
    return jsonPrivateNoStore({ home: await enhancePet(student, petId) });
  } catch (error) {
    const known = petErrorResponse(error);
    if (known) return jsonPrivateNoStore(known.body, { status: known.status });
    console.error("[POST /api/student/pets/:petId/enhance]", error);
