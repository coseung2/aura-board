/**
 * /share/[shareToken] — Public read-only board view.
 *
 * Anyone with the shareToken can view the board without authentication.
 * Renders the board in read-only mode (no edit controls, no drag, no add).
 */
import { notFound } from "next/navigation";
import { db } from "@/lib/db";
import { tokensEqual } from "@/lib/share/tokens";
import { BoardShareView } from "@/components/share/BoardShareView";
...
export default async function ShareBoardPage({
  ...
  const cardProps = cards.map((c) => ({
    ...
    authors: (c as any).authors?.map((a: any) => ({
      id: a.id,
